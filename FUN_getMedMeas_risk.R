getMedMeas_risk <- function(longdata, tau, model, transition.mat, CI=FALSE){
  ...
  res0 <- aggregate(risk_byID_0[,3:ncol(risk_byID_0)], list(trueArm=risk_byID_0[,2]), mean)
  res1 <- aggregate(risk_byID_1[,3:ncol(risk_byID_1)], list(trueArm=risk_byID_1[,2]), mean)
  
  # compute mediation measures according to risk difference for each time point
...
index <- 0
  ...
  for(t in tau){
    ...
    for(b in 1:B){
      ...
      n_C		<- nrow(risk_byID_0[which(risk_byID_0[,2]==0),])
      n_E		<- nrow(risk_byID_0[which(risk_byID_0[,2]==1),])
  	  ...
  	  ViPi_S1M1 	<- Ve*risk_byID_1[which(risk_byID_1[,2]==1), index+2]
  	  ViPi_S0M0 	<- Vc*risk_byID_0[which(risk_byID_0[,2]==0), index+2]
  	  ViPi_S1M0 	<- Vc*risk_byID_1[which(risk_byID_1[,2]==0), index+2]
